...
public class ImplMateBestPeer implements GaFlowMate {
	...
	public void crossover(GaPopulation population) {
		...
		for (int i = 0, j = 1, len = prevChromosomes.size(); i < len; i = i + 2, j = j + 2) {
			try {
				if (j < len) {
					this.multiply(prevChromosomes, nextChromosomes, i, j);
				} else {
					this.multiply(prevChromosomes, nextChromosomes, i);
				}
			} catch (Exception e) {
	...
	private void multiply(List<GaChromosome> prevChromosomes, List<GaChromosome> nextChromosomes, int index1, int index2) throws InstantiationException, IllegalAccessException, ClassNotFoundException {
		...
		List<GaChromosome> candidates = this.sortChromosomes(parent1, parent2, child1, child2);
		nextChromosomes.add(candidates.get(0));
		nextChromosomes.add(candidates.get(1));
	}
	
	private void multiply(List<GaChromosome> prevChromosomes, List<GaChromosome> nextChromosomes, int index1) throws InstantiationException, IllegalAccessException, ClassNotFoundException {
		...
		List<GaChromosome> candidates = this.sortChromosomes(parent, child);
		nextChromosomes.add(candidates.get(0));
	}

	private List<GaChromosome> sortChromosomes(GaChromosome... chromosomes) {
